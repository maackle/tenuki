...
BoardState.prototype = {
  copyWithAttributes: function copyWithAttributes(attrs) {
    var retrieveProperties = function retrieveProperties(_ref2) {
      ...
      var pass = _ref2.pass;
      var blackPassStones = _ref2.blackPassStones;
      var whitePassStones = _ref2.whitePassStones;
      var intersections = _ref2.intersections;
      var blackStonesCaptured = _ref2.blackStonesCaptured;
      var whiteStonesCaptured = _ref2.whiteStonesCaptured;
      var capturedPositions = _ref2.capturedPositions;
      var koPoint = _ref2.koPoint;
      var boardSize = _ref2.boardSize;
  ...
  _simpleKoPoint: function _simpleKoPoint() {
    ...
    if (this.playedPoint) {
      ...
      var y = _playedPoint.y;
      var x = _playedPoint.x;
...
Client.prototype = {
  _setup: function _setup(_ref) {
    ...
    var player = _ref.player,
        _ref$gameOptions = _ref.gameOptions,
        gameOptions = _ref$gameOptions === undefined ? {} : _ref$gameOptions,
        hooks = _ref.hooks;
      ...
      var defaultRendererHooks = {
        handleClick: function handleClick(y, x) {
          if (_this.isOver()) {
            ...
            var ok = _this.playAt(y, x);
            if (ok) _this.callbacks.postMove({ x: x, y: y });
          }
        },
...
Region._startingAt = function (boardState, y, x) {
  ...
  var _boardState$partitionTraverse = boardState.partitionTraverse(startingPoint, function (neighbor) {
    return neighbor.sameColorAs(startingPoint);
  });

  var _boardState$partitionTraverse2 = _slicedToArray(_boardState$partitionTraverse, 2);

  var includedPoints = _boardState$partitionTraverse2[0];
  var boundaryPoints = _boardState$partitionTraverse2[1];
...
Region.allFor = function (boardState) {
  ...
  boardState.intersections.forEach(function (point) {
    if (checkedPoints.indexOf(point) > -1) {} else {
      var _boardState$partitionTraverse3 = boardState.partitionTraverse(point, function (neighbor) {
        return neighbor.sameColorAs(point);
      });

      var _boardState$partitionTraverse32 = _slicedToArray(_boardState$partitionTraverse3, 2);

      var regionPoints = _boardState$partitionTraverse32[0];
      var _ = _boardState$partitionTraverse32[1];
...
Region.prototype = {
  ...
  territoryColor: function territoryColor() {
    ...
    var _Region$_startingAt32 = _slicedToArray(_Region$_startingAt3, 2);

    var _ = _Region$_startingAt32[0];
    var boundaryPoints = _Region$_startingAt32[1];
    ...
    return this.intersections.some(function (i) {
      return [[[0, 0], [1, 0], [2, 0], [2, 1]], [[-1, 2], [0, 0], [0, 1], [0, 2]], [[0, 0], [0, 1], [1, 1], [2, 1]], [[-1, 0], [-1, 1], [-1, 2], [0, 0]], [[-2, 1], [-1, 1], [0, 0], [0, 1]], [[0, 0], [1, 0], [1, 1], [1, 2]], [[0, -1], [0, 0], [1, -1], [2, -1]], [[-1, -2], [-1, -1], [-1, 0], [0, 0]]].some(function (expectedPoints) {
        return expectedPoints.every(function (_ref3) {
          ...
          var yOffset = _ref32[0];
          var xOffset = _ref32[1];
  ...
  _isKoViolation: function _isKoViolation(y, x, boardState, existingStates) {
    ...
    if (this.koRule === "simple") {
      ...
      (function () {
        ...
        if (_this.koRule === "positional-superko") {
          isKoViolation = hasDuplicatePosition(function () {
            return true;
          });
        } else if (_this.koRule === "situational-superko") {
          isKoViolation = hasDuplicatePosition(function (state) {
            return state.color === newState.color;
          });
        } else if (_this.koRule === "natural-situational-superko") {
          isKoViolation = hasDuplicatePosition(function (state) {
            return !state.pass && state.color === newState.color;
          });
        } else {
          throw new Error("Unimplemented ko rule " + _this.koRule);
        }
      })();
    }
...
var constructSVG = function constructSVG(renderer, boardState, _ref2) {
  ...
  var smallerStones = _ref2.smallerStones;
  var flatStones = _ref2.flatStones;
...
SVGRenderer.prototype.generateBoard = function (boardState, _ref3) {
  ...
  var smallerStones = _ref3.smallerStones;
  var flatStones = _ref3.flatStones;
